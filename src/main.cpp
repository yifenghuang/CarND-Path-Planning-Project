...
int main()
{
  ...
  h.onMessage([&ref_vel, &cur_lane, &num_units_in_current_lane, &map_waypoints_x,&map_waypoints_y,&map_waypoints_s,&map_waypoints_dx,&map_waypoints_dy](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length,
                     uWS::OpCode opCode) {
        ...
        if (event == "telemetry")
        {
          //j[1] is the data JSON object
            ...
            for(int i=0;i<sensor_fusion.size();i++)
            {
              ...
              // if a car is in front of me set the too_close flag
              if((check_car_lane == cur_lane) && (check_car_s-car_s)>0.0 && (check_car_s-car_s)<30.0)
              {
                too_close = true;
              }

              // if car exists in nearby lanes,set the lane shift flag to false
            ...
            //if car is too close to front car, first slow down and try lane changing if the condition is ok
            if(too_close)
            {
              ref_vel -= .224;
              for (int i = 0; i < 3; i++)
              {
                if (laneFlag[i] && if2lane(cur_lane, i) && (num_units_in_current_lane > 10))
                {
                  cur_lane = i;
            ...
            {
                //redefine reference state as previous
                ...
                //use 2 points that make the path tangent to the end point of previous path
            ...
            for(int i=1;i<= 50 - previous_path_x.size();i++)
            {
                ...
                //rotate back to normal to global coordinates
